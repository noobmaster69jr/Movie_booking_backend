...
validateBookingRequestBody = async (req, res, next) => {
  if (!req.body.theatreId) {
    return res
      .status(400)
      .send({ message: "Failed! Theatre id is not provided !" });
  }
  ...
  if (!req.body.movieId) {
    return res
      .status(400)
      .send({ message: "Failed! Movie id is not provided !" });
  }
  ...
  if (!savedTheatre.movies.includes(req.body.movieId)) {
    return res
      .status(400)
      .send({
        message: "Failed! movie id is not available in the given theatre!",
      });
  }
  ...
  if (!req.body.timings) {
    return res
      .status(400)
      .send({ message: "Failed! Timings is not provided !" });
  }
